...
□ Function
  ○ MainActivity → Background Thread → BackgroundHandler
...
public class BackgroundThread extends Thread {
    ...
    public BackgroundThread(MainHandler _mainThreadHandler, int _id) {
        ...
        //UI접근이 가능한 Handler 선언
        this.mainThreadHandler = _mainThreadHandler;

        //대상이 되는 PrograssBar 객체를 갖고 온다.
        id = _id;

        //Thread의 생성자에 OtherThreadHandler를 정의하게 되면
        //UI가 생성되어있는 MainThread에 정의한 것과 똑같으므로
        //별도의 Thread에서의 작업을 원할 시 run()메소드에 정의
